...
def load_feature_info():
    ...
    try:
        ...
        return {
            'group1': {
                'feature_names': ['suction', 'clay', 'silt', 'sand', 'dd', 'BC', 'temperature', 'Biochar_type_combined'],
...
def batch_predict_group1(model, data_df, feature_info):
    ...
    # 获取特征信息和类别
    biochar_categories = feature_info.get('biochar_categories', ["农业废弃物", "林业残余物", "畜禽粪便", "城市污泥", "其他"])
...
def validate_batch_data(data_df, model_type, feature_info):
    ...
    if model_type == 'group1' and 'Biochar_type_combined' in data_df.columns:
        invalid_types = data_df[~data_df['Biochar_type_combined'].isin(biochar_categories)]['Biochar_type_combined'].unique()
...
def display_single_prediction_interface(models, model_type, model_info, feature_info):
    ...
    if model_type == 'group1':
        ...
        biochar_categories = model_info.get('biochar_categories', ["农业废弃物", "林业残余物", "畜禽粪便", "城市污泥", "其他"])
            ...
            if st.button("开始批量预测", type="primary", use_container_width=True):
                with st.spinner("正在进行批量预测..."):
                    ...
                    if len(valid_predictions) > 0:
                        ...
                        ax.axvline(valid_predictions.mean(), color='red', linestyle='--', linewidth=2, label=f'均值: {valid_predictions.mean():.3f}')
                        ax.axvline(valid_predictions.median(), color='green', linestyle='--', linewidth=2, label=f'中位数: {valid_predictions.median():.3f}')
